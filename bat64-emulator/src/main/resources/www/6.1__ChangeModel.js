



var g_currentModel = "";
var g_ModelNameIndex = -1;
var g_ModelNamePreIndex = -1;


initPage();

function initPage(){

	GetTd(getCurrentModelName(InitDataPostArgs), g_InitEvent);
	GetTd(getModelListObject(InitDataPostArgs, 1), g_InitEvent, "get");


	setInterval(JsonFunction, 250);
}



function getModelListObject(InitDataPostArgs, number){
	if(typeof InitDataPostArgs == "undefined"){
		InitDataPostArgs = new Object();
	}

	modelManagement = new Object();
	list = new Object();
	item = new Array();
	list.Items = item;
	list.New = number;

	modelManagement.List = list;

	InitDataPostArgs["ModelManagement"] = modelManagement;

	return InitDataPostArgs;
}


function onEVENT_INIT(e){
	try{
		if(typeof e.EventData.ModelName != "undefined"){
			g_currentModel = e.EventData.ModelName;
		}
		else{
			modelList = e.EventData.ModelManagement.List.Items;

			if( modelList.length != 0){
				for(var i = 0; i < modelList.length; i++){
					ModelIsValid = modelList[i].IsValid;
					ModelName = modelList[i].ModelName;

					if(ModelIsValid == 1){
						ModelIMG      = modelList[i].Image;
						ModelInfo     = modelList[i].InfoText;
						ModelType     = modelList[i].Type;
						ModelCategory = modelList[i].Category.Name;

						if(ModelIMG == ""){
							switch(ModelType){
								
								
								
								
								default:			ModelIMG = CONST_MODELICON_DEFAULT_small;
							}
						}
						else{
							ModelIMG = "user-space/image/" + ModelIMG;
						}
					}
					else{
						
						ModelIMG = "data:image/png;base64,";
						ModelInfo = 'Modelldatei beschädigt!';
						ModelType = "--";
						ModelCategory = "--";
					}

					if(ModelName == g_currentModel){
						currentModelClass = "current_model";
						g_ModelNamePreIndex = i;
					}
					else{
						currentModelClass = "";
					}

					$('#scrollContainerInnerVertical').append(getRowOfModelList(i, ModelIMG, ModelName, ModelInfo, ModelType, ModelCategory, currentModelClass, ModelIsValid));
				}

				if(e.EventData.ModelManagement.List.New == 1){
					initScrollbars('List_Container');
				}
				else{
					ScrollRefresh();
				}
				GetTd(getModelListObject(InitDataPostArgs, 0), g_InitEvent, "get");
			}
		}
	}catch(err){
		onError(err, "Error Init: ", false);
	}
}


function onEVENT_SET(e){
	try{
		if(e.cmd == "tree-open"){


			
			
			

			
			window.location.href = "6.0__ModelManagement.html";

		}
	}catch(err){
		onError(err, "Error Setdata: ", false);
	}
}



function getRowOfModelList(i, ModelIMG, ModelName, ModelInfo, ModelType, ModelCategory, currentModelClass, ModelIsValid){
	var htmlInnerContainer = '' +
		'<div id="Index_' + i + '" class="list_content_row ' + currentModelClass + '" onClick=\'changeModel("' + ModelName + '", '+ i + ', ' + ModelIsValid + ');\'>' +
			'<div class="model_pic"><img id="Model__0_Pic" src="' + ModelIMG + '" width="54" height="54" alt="" draggable="false"/></div>' +
			'<div class="model_name">' + ModelName + '</div>' +
			'<div class="model_info">' + ModelInfo + '</div>' +
			'<div class="model_type">' + ModelType + '</div>' +

		'</div>';

	return 	htmlInnerContainer;
}


function changeModel(ModelName, index, isValid){
	if(isValid == 0){
		showDialogbox("error", 'Achtung, Ihre ausgewählte' + ' "' + ModelName + '" ' + 'Modelldatei ist beschädigt und kann nicht ausgewählt werden.', ModelName, index);
	}
	else{
		if(g_isRfConnected){
			showDialogbox("changeModelRfConnected", "rfConnected", ModelName, index);
		}
		else{
			submitSET(index, ModelName);
		}
	}
}

function openTree(TdJson){
	$('#Index_' + g_ModelNamePreIndex).removeClass('current_model');
	$('#Index_' + g_ModelNameIndex).addClass('current_model');

	g_ModelNamePreIndex = g_ModelNameIndex;
	g_currentModel = TdJson["tree-open"]["ModelName"];
}


function submitSET(i, tagId){
	g_ModelNameIndex = i;
	cmd = "tree-open";
	ModelName = "ModelName";

	xmlObj = {};
	xmlObj[cmd] = {};
	xmlObj[cmd][ModelName] = tagId;

	
	showDialogbox("actionWait", 'Modellwechsel wird durchgeführt...');
	GetTd(xmlObj, g_SetEvent, cmd);
}
