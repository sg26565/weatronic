...
var g_VoiceAndSoundAlertIndex 	= g_GET_Parameter.VoiceAndSoundAlertIndex;
var g_scrollIndex = 0;
	...
	try{
		...
		if(typeof e.EventData.get == "undefined"){
			...
			for(i = 0; i < g_List_Count; i++){
				g_List_Indices[i] = e.EventData.get.SoundMode.Alerts[i].Index;
				htmlOuterContainer += getRowRD(g_List_Indices[i], e.EventData.get.SoundMode.Alerts[i].AudioItem.TelemetryIDInfo.Name, 'Alarm', g_List_Indices[i]);
				
				if(g_List_Indices[i] == g_VoiceAndSoundAlertIndex){
					g_scrollIndex = i;
				}
			}	
			...
			for(i = 0; i < g_List_Count; i++){
				Index         		= e.EventData.get.SoundMode.Alerts[i].Index;
				Item		   		= e.EventData.get.SoundMode.Alerts[i].AudioItem.TelemetryIDInfo.Name;
				...
				telemetryID	   		= e.EventData.get.SoundMode.Alerts[i].AudioItem.TelemetryIDInfo.ID;
				...
				setHTML("Container_" + Index, getRowOfAlertsList(Index, Item, telemetryID));
			...
			if(typeof g_VoiceAndSoundAlertIndex != "undefined"){
...
function getRowOfAlertsList(Index, Item, telemetryID){
	classSetupNoEdit = "";
	...
	var htmlInnerContainer = '' +
								'<div id="Alerts__' + Index + '_ID" class="list_voicesoundsalert_item" onClick=\'window.location.href="1.16.1__TelemetryDataAssignment.html?PageType=Alerts&ModeIndex=' + g_ModeIndex + '&Item=' + Index + '"\'>' + Item + '</div>' +
								...
								'<div id="' + telemetryID + '" class="list_voicesoundsalert_live">-.-</div>' +
								'<div class="list_voicesoundsalert_setup ' + classSetupNoEdit + '">' +
									'<a href="1.7.2.1__VoiceAndSoundsAlertsSetup.html?SetupIndices=' + g_ModeIndex + '___' + Index + '" draggable="false">' +
...
function submitARD(cmd, num){
	...
	xmlObj[cmd][ModelName][ListType]["Alert"] = num;
...
function createAddItem(TdJson){
	...
	window.location.href="1.16.1__TelemetryDataAssignment.html?PageType=Alerts&ModeIndex=" + g_ModeIndex + "&Item=" + Index;
}
