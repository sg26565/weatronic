...
var g_LimiterIndex = g_GET_Parameter.LimiterIndex;
var g_scrollIndex = 0;
...
function initPage(){
	...
	GetTd(getLimiterObject(InitDataPostArgs), g_InitEvent);
...
function getLimiterObject(InitDataPostArgs){
	...
	limiterItems = new Array(Item);

	Limiter = new Object();
	Limiter.Items = "ALL_USED";
...
function onEVENT_INIT(e){
	try{
		...
		g_List_Indices = e.EventData.SortIndeces.Limiters;
		...
		for(i = 0; i < g_List_Count; i++){
			...
			for(var j = 0; j < g_List_Count; j++){
				if(Index == e.EventData.Limiter.Item[j].Index){
					Name = e.EventData.Limiter.Item[j].Name;
					break;
				}
			}

			htmlOuterContainer += getRowRD(Index, Name, 'Limiter', i);
			
			if(Index == g_LimiterIndex){
				g_scrollIndex = i;
			}
		}
		...
		for(i = 0; i < g_List_Count; i++){
			Index        = e.EventData.Limiter.Item[i].Index;
			LimiterName  = e.EventData.Limiter.Item[i].Name;
			FunctionName = e.EventData.Limiter.Item[i].FunctionName;

			
			setHTML("Container_" + Index, getRowOfLimitersList(Index, LimiterName, FunctionName));
		}
...
function getRowOfLimitersList(Index, LimiterName, FunctionName){

	var htmlInnerContainer = '' +
		'<!-- Limiter Name -->' +
		'<div id="Limiter__' + Index + '_Name" class="list_limiter_name Name keypad-tag" onClick=\'showKeypad("Limiter__' + Index + '_Name");\'>' + LimiterName + '</div>' +

		'<!-- Function Name -->' +
		'<div id="Limiter__' + Index + '_Name" class="list_limiter_name Name no_edit">' + FunctionName + '</div>' +

		'<!-- Limiter Setup -->' +
		'<div id="Limiter__' + Index + '_Setup" class="list_limiter_setup">' +
			'<a href="1.6.1__LimiterSetup.html?LimiterIndex=' + Index + '" draggable="false">' +
...
function createAddItem(TdJson){
	...
	Index = TdJson.add.Limiter.Index;
	LimiterName = TdJson.add.Limiter.Name;

	g_List_Indices.push(Index);
	g_List_Count = g_List_Indices.length;

	var newChild = getRowRD(Index, LimiterName, 'Limiter', (g_List_Count - 1)); 
	$("#scrollContainerInnerVertical").append(newChild);

	
	setHTML('Container_' + Index, getRowOfLimitersList(Index, LimiterName, "--"));
