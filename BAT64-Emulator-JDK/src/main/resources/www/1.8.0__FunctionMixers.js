...
var g_MixerIndex = g_GET_Parameter.MixerIndex;
var g_scrollIndex = 0;
...
function initPage(){
	...
	GetTd(getMixerObject(InitDataPostArgs), g_InitEvent);
...
function onEVENT_INIT(e){
	try{
		...
		g_List_Indices = e.EventData.SortIndeces.Mixers;
		...
		for(i = 0; i < g_List_Count; i++){
			...
			for(var j = 0; j < g_List_Count; j++){
				if(Index == e.EventData.Mixer.Item[j].Index){
					Name = e.EventData.Mixer.Item[j].Name;
					break;
				}
			}
			htmlOuterContainer+= getRowRD(Index, Name, 'Mischer', i);
			
			if(Index == g_MixerIndex){
				g_scrollIndex = i;
			}
		}
		...
		for(i = 0; i < g_List_Count; i++){
			Index = e.EventData.Mixer.Item[i].Index;
			MixerName = e.EventData.Mixer.Item[i].Name;
			FromFunctionName = e.EventData.Mixer.Item[i].FromFunctionName;
			ToFunctionName = e.EventData.Mixer.Item[i].ToFunctionName;

			
			setHTML("Container_" + Index, getRowOfMixersList(Index, MixerName, FromFunctionName, ToFunctionName));
		}

		
		if(typeof g_MixerIndex != "undefined"){
...
function createAddItem(TdJson){
	...
	Index = TdJson.add.Mixer.Index;
	MixerName = TdJson.add.Mixer.Name;
	FromFunctionName = TdJson.add.Mixer.FromFunction;
	ToFunctionName = TdJson.add.Mixer.ToFunction;

	g_List_Indices.push(Index);
	g_List_Count = g_List_Indices.length;

	var newChild = getRowRD(Index, MixerName, 'Mischer', (g_List_Count-1));
